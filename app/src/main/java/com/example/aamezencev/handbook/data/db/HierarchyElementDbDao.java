...
public class HierarchyElementDbDao extends AbstractDao<HierarchyElementDb, Long> {
    ...
    private Query<HierarchyElementDb> hierarchyElementDb_ChildrenHierarchyElementsQuery;
    ...
    /** Internal query to resolve the "childrenHierarchyElements" to-many relationship of HierarchyElementDb. */
    public List<HierarchyElementDb> _queryHierarchyElementDb_ChildrenHierarchyElements(Long parentId) {
        synchronized (this) {
            if (hierarchyElementDb_ChildrenHierarchyElementsQuery == null) {
                QueryBuilder<HierarchyElementDb> queryBuilder = queryBuilder();
                queryBuilder.where(Properties.ParentId.eq(null));
                hierarchyElementDb_ChildrenHierarchyElementsQuery = queryBuilder.build();
            }
        }
        Query<HierarchyElementDb> query = hierarchyElementDb_ChildrenHierarchyElementsQuery.forCurrentThread();
    ...
    protected String getSelectDeep() {
        if (selectDeep == null) {
            ...
            builder.append(',');
            SqlUtils.appendColumns(builder, "T0", daoSession.getHierarchyElementDbDao().getAllColumns());
            builder.append(',');
            SqlUtils.appendColumns(builder, "T1", daoSession.getDataHierarchyDbDao().getAllColumns());
            builder.append(" FROM HIERARCHY_ELEMENT_DB T");
            builder.append(" LEFT JOIN HIERARCHY_ELEMENT_DB T0 ON T.\"PARENT_ID\"=T0.\"_id\"");
            builder.append(" LEFT JOIN DATA_HIERARCHY_DB T1 ON T.\"DATA_HIERARCHY_ID\"=T1.\"_id\"");
            builder.append(' ');
    ...
    protected HierarchyElementDb loadCurrentDeep(Cursor cursor, boolean lock) {
        ...
        HierarchyElementDb parentHierarchyElement = loadCurrentOther(daoSession.getHierarchyElementDbDao(), cursor, offset);
        entity.setParentHierarchyElement(parentHierarchyElement);
        offset += daoSession.getHierarchyElementDbDao().getAllColumns().length;
